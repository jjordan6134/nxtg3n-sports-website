...
export default async function AthleteMediaKitPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="media-kit-page mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
    ...
    <section className="border-t border-white/10 py-10" aria-labelledby="kit-video-social"><h2 id="kit-video-social" className="text-2xl font-black text-white">Verified video and social links</h2>{videoItems.length ? <div className="mt-6 grid gap-6 md:grid-cols-2">{videoItems.map((item) => <EmbeddedMediaPlayer key={item.id} item={item} athleteSlug={athlete.slug} location="media_kit" />)}</div> : null}{linkedItems.length ? <div className="mt-6 grid gap-4 md:grid-cols-2">{linkedItems.map((item) => <SocialMediaCard key={item.id} item={item} athleteSlug={athlete.slug} location="media_kit" />)}</div> : null}{!videoItems.length && !linkedItems.length ? <p className="mt-3 text-[#C7CCD6]">No verified athlete video or social links are currently listed.</p> : null}</section>

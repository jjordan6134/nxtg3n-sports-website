...
import { MediaDevelopmentState } from "@/components/media-experience";
...
export default async function AthleteMediaKitPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const socialItems = items.filter((item) => item.platform && ["instagram", "tiktok", "x"].includes(item.platform));
  ...
  return <main className="media-kit-page mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
    ...
    <section className="border-t border-white/10 py-10" aria-labelledby="kit-video-social"><h2 id="kit-video-social" className="text-2xl font-black text-white">Verified video and social links</h2><p className="mt-3 text-[#C7CCD6]">{videoItems.length || socialItems.length ? "Verified links available in the media archive." : "No verified athlete video or social links are currently listed."}</p></section>

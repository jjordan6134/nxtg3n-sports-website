...
export function VideoHighlight({ item, athleteSlug, location }: { item: MediaItem; athleteSlug: string; location: string }) {
  ...
  return <article className="rounded-[2rem] border border-white/10 bg-[#101722] p-6">
    ...
    <div className="mt-5 aspect-video overflow-hidden rounded-2xl border border-white/10 bg-[#0B0E11]">
      {loaded && !failed ? <iframe src={embedUrl} title={item.title} className="h-full w-full" loading="lazy" sandbox="allow-scripts allow-same-origin allow-presentation" referrerPolicy="strict-origin-when-cross-origin" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen onError={() => setFailed(true)} /> : <button type="button" onClick={() => { setLoaded(true); trackMediaEvent({ name: "video_preview_click", properties: { media_id: item.id, athlete_slug: athleteSlug, platform, location } }); trackMediaEvent({ name: "video_play", properties: { media_id: item.id, athlete_slug: athleteSlug, platform, location } }); }} aria-label={`Play ${item.title}`} className="group relative flex h-full w-full items-end overflow-hidden bg-cover bg-center text-left text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#2AFF7D]" style={item.thumbnail ? { backgroundImage: `url(${item.thumbnail})` } : undefined}><span className="absolute inset-0 bg-[#0B0E11]/70 transition group-hover:bg-[#0B0E11]/55" /><span className="relative z-10 flex w-full items-end justify-between gap-4 p-5"><span><span className="block text-xs font-semibold uppercase tracking-[0.2em] text-[#2AFF7D]">{item.type}</span><span className="mt-2 block text-lg font-black text-white">{item.title}</span></span><span className="flex h-16 w-16 shrink-0 items-center justify-center rounded-full border-2 border-[#2AFF7D] bg-[#0B0E11]/80 text-2xl text-white" aria-hidden="true">▶</span></span></button>}
    </div>
